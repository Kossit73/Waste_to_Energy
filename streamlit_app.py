...
def _render_ai_settings(payload: Dict[str, Any], container: Optional[DeltaGenerator] = None) -> None:
    ...
    if submitted:
        ...
        st.success("AI configuration updated. Press Recalculate to apply it to model results.")
        _rerun()
...
WTE_STATE_KEYS: list[str] = []


def main() -> None:
    global WTE_STATE_KEYS
    ...
    active_page = st.radio(
        ...
        page_labels,
        horizontal=True,
        key="wte_active_workspace_section",
        label_visibility="collapsed",
    )
    run_controls = st.empty()


    if active_page == "Input Landing":
    ...
    if active_page == "Revenue & Production":
    ...
    if active_page == "Operations & Working Capital":
    ...
    if active_page == "Financing & Taxes":
    ...
    accounts_receivable = _ensure_state_df(
        "accounts_receivable", accounts_receivable_defaults
    )
    inventory_payable = _ensure_state_df(
        "inventory_payable", inventory_payable_defaults
    )
    ...
    draft_fingerprint = hashlib.sha256(repr(user_inputs).encode("utf-8")).hexdigest()
    active_bundle = st.session_state.get("wte_active_result")
    ...
    if run_clicked:
        cache = st.session_state.setdefault("wte_model_result_cache", {})
        active_bundle = cache.get(draft_fingerprint)
        cache_hit = isinstance(active_bundle, dict)
        if not cache_hit:
            with st.spinner("Running waste-to-energy model..."):
                run_inputs = copy.deepcopy(user_inputs)
                run_results = cashflow_model(run_inputs)
                run_results["ai_settings"] = copy.deepcopy(
                    st.session_state.get("ai_settings", DEFAULT_AI_SETTINGS)
                )
                active_bundle = {
                ...
                cache[draft_fingerprint] = active_bundle
        st.session_state["wte_active_result"] = active_bundle
        st.session_state["wte_last_run_fingerprint"] = draft_fingerprint
        st.session_state["input_snapshot"] = copy.deepcopy(active_bundle["inputs"])
        st.session_state["results_snapshot"] = copy.deepcopy(active_bundle["results"])
    ...
    st.session_state["wte_model_results_stale"] = results_stale
    if isinstance(active_bundle, dict) and results_stale:
        st.warning(
            "Draft inputs have changed. Outputs, scenarios, and exports still use "
    ...
    user_inputs = copy.deepcopy(active_bundle["inputs"])
    ...
    if active_page == "Key Metrics Dashboard":
    ...
    if active_page == "Financial Performance":
    ...
    if active_page == "Financial Position":
    ...
    if active_page == "Cash Flow Statement":
    ...
    if active_page == "Sensitivity Analyses":
    ...
    if active_page == "Scenario / Ifs":
        ...
        analytics_cache = st.session_state.setdefault("wte_analytics_cache", {}).setdefault(
            active_fingerprint, {}
        )
        scenario_cache_key = hashlib.sha256(
            scenario_signature.encode("utf-8")
        ).hexdigest()
        scenario_results = analytics_cache.get(f"scenarios:{scenario_cache_key}")
        if not scenario_names:
            st.info("Add scenarios to compare outcomes.")
        elif st.button("Run Scenario Comparison", key="run_wte_scenarios", type="primary"):
            with st.spinner("Evaluating waste-to-energy scenarios..."):
                computed_results: List[Dict[str, float]] = []
                ...
                scenario_results = computed_results
                analytics_cache[f"scenarios:{scenario_cache_key}"] = scenario_results

        st.subheader("Scenario Comparison")
        if scenario_results:
            scenario_frame = pd.DataFrame(scenario_results)
            st.dataframe(scenario_frame.round(4), use_container_width=True)
            st.bar_chart(scenario_frame.set_index("Scenario"))
        else:
        ...
        export_cache_key = f"{active_fingerprint}:{scenario_cache_key}:{selected_scenario}"
            ...
            if not excel_bytes:
                if st.button("Prepare Excel Model", key=f"prepare_excel_{selected_scenario.lower()}"):
                    with st.spinner("Preparing Excel workbook..."):
                        ...
                        export_model.scenario = selected_scenario
                        excel_bytes = generate_excel_bytes(
                            export_model, scenario_payload_results, selected_scenario
                        )
                        st.session_state["model_results"] = (
                            export_model,
                            scenario_payload_results,
                        )
    ...
    if active_page == "Break-Even & Payback":
...
def set_state(state: dict) -> None:
    """Restore saved assumptions before ``main()`` initialises its widgets."""
    for key, value in state.items():
        if not WTE_STATE_KEYS or key in WTE_STATE_KEYS:
            st.session_state[key] = value
